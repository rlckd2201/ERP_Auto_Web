...
## Corpus Check
- 229 files · ~245,264 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1180 nodes · 3776 edges · 29 communities detected
- Extraction: 84% EXTRACTED · 16% INFERRED · 0% AMBIGUOUS · INFERRED: 607 edges (avg confidence: 0.78)
...
3. `get_invoice()` - 35 edges
4. `WehagoHandler` - 32 edges
5. `init_db()` - 25 edges
6. `add_invoice_log()` - 25 edges
7. `UplusEdocuHandler` - 24 edges
8. `update_invoice_json()` - 24 edges
9. `SmartBillHandler` - 23 edges
10. `UplusEdocuHandler` - 22 edges
...
Cohesion: 0.04
Nodes (45): _aggregate_expense_items(), _appdata_template_candidates(), _build_expense_report_text(), _clean_expense_item_name(), _clean_path(), _copy_or_merge_doc(), _ensure_appdata_expense_template(), _excel_process_ids() (+37 more)
...
Cohesion: 0.09
Nodes (93): accountOptions(), addLog(), agentConnectedFromSetup(), agentUpdateRequiredFromSetup(), applyDetailMode(), approvalPaths(), approvalStatusText(), autoStartAgentAfterLogin() (+85 more)

### Community 3 - "Community 3"
Cohesion: 0.07
Nodes (48): _add_installer_file(), _add_installer_tree(), _agent_bootstrap_script(), _agent_cmd_launcher(), _agent_exe_launcher(), _agent_installer_script(), _agent_self_contained_cmd(), api_delete_invoice() (+40 more)

### Community 4 - "Community 4"
Cohesion: 0.1
Nodes (75): api_agent_job_complete(), api_agent_job_expense_report_upload(), api_agent_job_voucher_upload(), api_analyze_purchase(), api_create_manual_purchase_invoice(), api_get_invoice_output_set(), api_update_purchase_analysis(), api_upload_erp_voucher() (+67 more)
...
Cohesion: 0.17
Nodes (34): _check_playwright_runtime(), fetch_approval_documents(), build_purchase_erp_payload(), build_regular_erp_payload(), _clean_text(), _configure_pyautogui_for_server(), _corp_codes(), _extract_invoice_date() (+26 more)

### Community 9 - "Community 9"
Cohesion: 0.11
Nodes (42): api_agent_heartbeat(), api_agent_job_event(), api_agent_next_task(), api_agent_setup_install_complete(), api_login(), api_setup_install(), api_setup_installer(), api_setup_printers() (+34 more)
...
Cohesion: 0.14
Nodes (34): _agent_bundle_hash(), _apply_server_setup_config(), _cert_cache_path(), _cert_store_has_thumbprint(), _cert_thumbprint(), _config_path(), _detect_printer_mapping(), _display_check() (+26 more)
...
- **Thin community `Community 22`** (9 nodes): `WEB v1 backend package.`, `__init__.py`, `__init__.py`, `__init__.py`, `__init__.py`, `__init__.py`, `__init__.py`, `__init__.py`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 23`** (9 nodes): `create_https_cert.py`, `create_https_cert.py`, `create_https_cert.py`, `create_https_cert.py`, `create_https_cert.py`, `create_https_cert.py`, `create_https_cert.py`, `main()`, `create_https_cert.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (9 nodes): `__init__.py`, `__init__.py`, `__init__.py`, `__init__.py`, `__init__.py`, `__init__.py`, `__init__.py`, `Backend maintenance tools.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 25`** (6 nodes): `__init__.py`, `__init__.py`, `__init__.py`, `__init__.py`, `__init__.py`, `Accounting automation WEB v1 package.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 28`** (1 nodes): `이 핸들러가 처리 가능한 URL인지 반환.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 29`** (1 nodes): `실제 크롤링 로직. result dict를 직접 채운다.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 30`** (1 nodes): `캐시된 드라이버 우선 탐색 → 없으면 ChromeDriverManager 자동 설치.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `BaseTaxInvoiceHandler` connect `Community 1` to `Community 18`, `Community 19`, `Community 12`, `Community 6`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `JobStore` connect `Community 3` to `Community 0`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
...
- **Are the 23 inferred relationships involving `get_invoice()` (e.g. with `_active_invoice_items()` and `claim_next_erp_task()`) actually correct?**
  _`get_invoice()` has 23 INFERRED edges - model-reasoned connections that need verification._
