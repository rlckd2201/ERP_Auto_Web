...
Agent는 전표 PDF를 먼저 보관하고, Windows 기본 프린터를 변경하지 않고 지정 프린터로 출력한다.
운영 기본 프린터는 `재정 프린터 (172.16.10.173)`이다.
...
메일 송신은 기존 회계업무 WEB과 같은 내부 SMTP `35.216.76.148:25`, 발신 주소 `admpdm@dae-seung.co.kr`, 표시명 `재정전표자동화 시스템`을 기본값으로 사용한다.
