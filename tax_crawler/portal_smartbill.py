...
def _smartbill_print_full_a4_pdf_base64(driver) -> str:
    """Print SmartBill exactly as a normal Chrome A4 portrait page.

    The header/footer intentionally mirrors Chrome's print output: date and
    page title at the top, source URL and page number at the bottom.  Keeping
    the full page is important because the blank lower area is part of the
    original SmartBill print layout.
    """
    header_template = """
    <div style="font-size:8px;width:100%;padding:0 12px;box-sizing:border-box;color:#000;">
      <span class="date"></span>
      <span class="title" style="position:absolute;left:0;right:0;text-align:center;"></span>
    </div>
    """
    footer_template = """
    <div style="font-size:8px;width:100%;padding:0 12px;box-sizing:border-box;color:#000;">
    ...
    pdf_base64 = (result or {}).get("data")
    if not pdf_base64:
        raise RuntimeError("Chrome Page.printToPDF returned no PDF data")
    return pdf_base64


def _normalize_smartbill_pdf_layout(pdf_path: Path) -> Path:
    """Compatibility hook that deliberately preserves the original full page.

    Older deployments cropped the invoice and enlarged it onto landscape A4.
    That changed the SmartBill print artifact.  Do not crop, rotate, resize or
    otherwise rewrite the PDF here.
    """
    return pdf_path
...
def _smartbill_form_post_print_save_pdf(self, driver, final_path):
    ...
    try:
        print(f"[{self.portal_name}] Chrome A4 portrait PDF create")
        pdf_base64 = _smartbill_print_full_a4_pdf_base64(driver)
        final_path.parent.mkdir(parents=True, exist_ok=True)
        with open(final_path, "wb") as f:
            f.write(base64.b64decode(pdf_base64))
